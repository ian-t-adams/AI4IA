...
@description('Model capacity profile. baseline is portable; production requires a complete reviewed catalog policy and explicit capacities with reserved headroom; operator-only maximum uses subscription-specific maxCapacity, falling back to baseline where no maximum was recorded. No profile is selected automatically.')
@allowed([
  'baseline'
  'production'
  'maximum'
])
param modelCapacityProfile string = 'baseline'
...
@description('Give each user their own Search index instead of one shared index filtered by user_id. Per-user makes the tier index limit a ceiling on users (15 basic / 50 standard S1 / 200 S2-S3). Switching this strands existing chunks in the previous index until documents are reindexed (POST /api/library/documents/reindex, which re-embeds without re-running the analyzer).')
...
@description('ID of the separately approved deletion cutover record in the existing sessions control partition. Empty by default. This template never creates or approves the record.')
...
var modelDeploymentsByRegion = [for r in regionList: flatten(map(deployableCatalog, m => map(filter(m.deployments, d => d.region == r.name), d => {
  ...
  version: d.version
  sku: d.sku
  capacity: selectedCapacity(d, modelCapacityProfile)
})))]
