...
@description('Model capacity profile. baseline uses the portable capacity values; maximum uses subscription-specific maxCapacity values generated by scripts/sync-model-capacity.py, falling back to baseline where no maximum was recorded.')
@allowed([
  'baseline'
  'maximum'
])
...
var modelDeploymentsByRegion = [for r in regionList: flatten(map(deployableCatalog, m => map(filter(m.deployments, d => d.region == r.name), d => {
  ...
  version: d.version
  sku: d.sku
  capacity: modelCapacityProfile == 'maximum' ? (d.?maxCapacity ?? d.capacity) : d.capacity
})))]
