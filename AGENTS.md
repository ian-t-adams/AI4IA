...
- `scripts` — catalog generators, validators, provisioning helpers, status snapshots, teardown/purge scripts, and azd hooks. `scripts/azure-cli.ps1` is a shared dot-sourced safety library for Azure-facing PowerShell, not a standalone entry point.
