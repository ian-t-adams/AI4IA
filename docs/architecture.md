...
`protocolVersion`: **`2025-06-18` remains the omitted-field default** for existing
and BYO records and catalog entries. Explicit **`2025-11-25`** uses the same
stateful lifecycle. The packaged Foundry Toolbox selects November based on its
observed live initialize response. Both stateful versions use `initialize`,
validate the exact configured version, send
`notifications/initialized`, and retain any valid negotiated session ID on
subsequent requests and notifications together with the selected version header.
A server returning a different or missing version fails
visibly rather than being called under a contradictory contract.
Paginated stateful lists retain one initialized, DNS-pinned session and use distinct
...
result types surface as errors. Stateful results may omit `resultType`; modern
...
All versions use the same no-redirect, bounded-response and DNS-pinned public
HTTPS transport. SSE returns on the matching final response; cancellation closes
the stream and, on either stateful path, also sends a bounded cancellation notification
with the original request ID/auth/session/version. Errors never downgrade protocols,
...
version header. Stateful routes reject modern mirrors and, when present, require
the exact catalog-selected version header; modern routes require the
...
The November stateful contract follows the official
...
The stateless contracts follow the official [versioning rules](https://modelcontextprotocol.io/specification/2026-07-28/basic/versioning),
...
APIM's preview MCP surface or Foundry Toolbox supports the stateless revision. Verify
