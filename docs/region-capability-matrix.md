...
| `mistral-document-ai-2512` / `mistral-ocr-4-0` | GA/Preview respectively; final-name East US 2 Global Standard deployments were verified `Succeeded`, and the governed catalog declares East US 2 + Sweden Central at capacity 1. | Dedicated Analyzer pathways, not chat models. Explicit selection, canonical Markdown normalization, deployment/page provenance, and page-based metering are implemented; Content Understanding remains the automatic default. Verify both Sweden deployments in the rollout. |
