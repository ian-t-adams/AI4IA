...
| Azure AI Search | `AI4IA_SEARCH_ENABLED`, `AI4IA_SEARCH_LOCATION` | `searchEnabled`, `searchLocation` | `AI4IA_SEARCH_ENDPOINT` when provisioned, `AI4IA_SEARCH_INDEX_NAME` (default `ai4ia-doc-chunks`), `AI4IA_SEARCH_SEMANTIC_RANKING` (default `true`) | Profile default `true`; region must have Search SKU capacity. Retrieval is hybrid vector + BM25, with the L2 semantic reranker on top when semantic ranking is on. **The service is provisioned on the `free` semantic plan (1,000 semantic queries/month).** Past that cap every semantic query fails; retrieval degrades to plain hybrid and the store suppresses further semantic attempts for a cooldown that starts at 5 minutes, doubles per consecutive failure to a 1-hour cap, and clears on the first success — so an exhausted quota costs at most one probe per hour instead of a failed round-trip on every query. Only service-level failures (403/408/429/5xx/transport) open that shared breaker; a request-specific rejection falls back for that query alone. Set `AI4IA_SEARCH_SEMANTIC_RANKING=false` to force plain hybrid, or move the service to a paid semantic plan in `infra/modules/search.bicep`. |
