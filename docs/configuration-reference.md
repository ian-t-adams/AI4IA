...
| Raw-file compute | none (runtime setting only) | none | `AI4IA_CODE_INTERPRETER_RAW_FILES_ENABLED` (default `false`), `AI4IA_CODE_INTERPRETER_MAX_RAW_FILE_BYTES` (default 25 MiB) | Requires library compute. When off, `run_code` sees only Content Understanding's **parsed text**; when on it uploads the document's **original bytes** to the sandbox so the model reads the real PDF/xlsx/csv. Falls back to parsed text on any unsupported type, oversize original, or upload failure, so enabling it cannot break an existing run. No Bicep parameter today — it is not reachable from a normal `azd` deploy. |
