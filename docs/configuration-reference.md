...
| Model capacity profile | `AI4IA_MODEL_CAPACITY_PROFILE` | `modelCapacityProfile` | `baseline` (portable default) or `maximum`. Maximum selects subscription-specific `maxCapacity` values generated by `scripts/sync-model-capacity.py`; regenerate after quota or regional availability changes. It can consume the full pool and leave no quota for another deployment. |
