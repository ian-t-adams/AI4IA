...
| API schema and interactive docs | `AI4IA_OPENAPI_ENABLED` | `apiOpenapiEnabled` | FastAPI `/openapi.json`, `/docs`, and `/redoc` remain available in local/dev. Production hides them by default and exposes them only when an operator explicitly sets this true. This reduces accidental surface exposure; it is not an authorization control or a security-through-obscurity claim. |
...
| Content Understanding Agentic analyzer | `AI4IA_CU_AGENTIC_ANALYZER_ID` | `cuAgenticAnalyzerId` | `AI4IA_CU_AGENTIC_ANALYZER_ID` | Existing, operator-created analyzer whose resolved `config.workflow` is `agentic.*`. Empty means unavailable. Provision validation requires preview and at least 400K TPM on the primary GPT-5.2 deployment; the checked-in production capacity is 50K, so Agentic remains off. |
| Library compute / export | `AI4IA_DOCUMENT_COMPUTE_ENABLED` | `documentComputeEnabled` | `AI4IA_DOCUMENT_COMPUTE_ENABLED` | Profile default `true`; requires document understanding. Code Interpreter endpoint/model default to primary Foundry + `gpt-5.4-mini-*` unless overridden. |
...
| Azure AI Search | `AI4IA_SEARCH_ENABLED`, `AI4IA_SEARCH_LOCATION` | `searchEnabled`, `searchLocation` | `AI4IA_SEARCH_ENDPOINT` when provisioned | Profile default `true`; region must have Search SKU capacity. |
...
| Web IQ search tools | `AI4IA_WEB_SEARCH_ENABLED`, `AI4IA_WEBIQ_API_KEY` (secret) | `webSearchEnabled`, `webIqApiKey` | `AI4IA_WEB_SEARCH_ENABLED`, `AI4IA_WEBIQ_API_KEY` (secret) | Profile default `true`. Requires an API key or an Entra managed identity entitled to Web IQ; otherwise calls fail with 401. |
