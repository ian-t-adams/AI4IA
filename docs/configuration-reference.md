...
| API schema and interactive docs | Bicep override only today | `apiOpenapiEnabled` | FastAPI `/openapi.json`, `/docs`, and `/redoc` remain available in local/dev. Production hides them by default and exposes them only when an operator explicitly sets this true. The runtime setting is `AI4IA_OPENAPI_ENABLED`; CI deployment does not forward it as a workflow input today. This reduces accidental surface exposure; it is not an authorization control or a security-through-obscurity claim. |
