...
| Video generation | `AI4IA_VIDEO_GENERATION_ENABLED` | `videoGenerationEnabled` | `AI4IA_VIDEO_BLOB_ACCOUNT_URL` when provisioned | Profile default `true`; requires a video-capable deployment and generated-media storage. Sora 2 uses the Azure OpenAI v1 `/videos` create/status/content API through SimpleL7Proxy and APIM; supported clip lengths are 4, 8, or 12 seconds. |
