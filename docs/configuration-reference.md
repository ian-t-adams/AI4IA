...
| Proxy App Configuration label | `AI4IA_PROXY_APPCONFIG_LABEL` | `proxyAppConfigLabel` | `AZURE_APP_CONFIG_LABEL`, `AZURE_APPCONFIG_ENDPOINT`, label-aware `Warm:Sentinel`, 30-second refresh | Postprovision reconciles the sentinel with Entra auth through the deployment identity's narrow store-scoped Data Owner role; the proxy remains Data Reader only. The proxy applies only `Warm:Sentinel` and four reviewed warm settings from the store (`CircuitBreaker:ErrorThreshold`, `CircuitBreaker:Timeslice`, `Request:DefaultTimeout`, `Request:DefaultTTLSecs`), at startup and on refresh. Every other key, including every `Cold:` key, is refused, and its setting comes from the Container App environment. See [the key policy](../proxy/README.md#app-configuration-key-policy). |
