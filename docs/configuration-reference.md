...
| --- | --- | --- | --- |
| Deployment posture | `AI4IA_APP_ENVIRONMENT` | `appEnvironment` | `dev` or `prod`; **defaults to `dev`**. Set it to `prod` for any real environment — a standup that only sets the *documented* flags deploys as `dev` and nothing fails. Reaches both the api and web container apps. |
...
| Content Understanding preview | checked-in parameter | `cuPreviewEnabled` | `AI4IA_CU_PREVIEW_ENABLED` | Enables only explicitly selected `2026-06-01-preview` capabilities. Production keeps the automatic/default analyzer on GA `2025-11-01`; synchronous Read/Layout are the preview choices exposed when this is true. |
...
| Foundry toolbox (bridge) | checked-in parameter | `enableFoundryToolbox` (+ `enableOfficialMcp`) | consumed via the official MCP plane; `AZURE_FOUNDRY_PROJECT_ENDPOINT` output feeds `scripts/provision-foundry-toolbox.py` | Toolbox is consumed as one `infra/mcp-servers.json` entry (no app runtime setting). The flag grants project-scoped "Foundry User" to the MCP APIM MI for invocation and to the nonempty deployment principal for asset reconciliation; the latter is provisioning-only. Preview. See [`foundry-toolbox.md`](foundry-toolbox.md). |
| Private tool catalog (API Center) | `AI4IA_API_CENTER_LOCATION` | `enablePrivateToolCatalog`, `apiCenterLocation` | IaC creates MCP APIs, versions, definitions, an APIM environment, and active deployments | Requires `enableOfficialMcp`; every catalog deployment uses API-Center-scoped definition/environment IDs and the APIM consumer URL, never a raw upstream. The nonempty deployment principal gets Azure API Center Data Reader at service scope for provisioning/catalog reconciliation. MCP inventory remains preview. See [`foundry-toolbox.md`](foundry-toolbox.md). |
