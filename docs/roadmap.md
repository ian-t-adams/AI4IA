...
| **P1** | **Proactive alerting is off.** `enableAlerts=false`; only App Insights Smart Detection is live, so there are no threshold alerts for API 5xx spikes or Cosmos throttling. | Wire `enableAlerts` + `alertEmail` into `deploy.yml`/repo variables (same pattern as `AI4IA_MEMORY_STORE`) and set a recipient. Reactive diagnosis via the admin dashboard already works. | `infra/modules/alerts.bicep`, [`telemetry.md`](./runbooks/telemetry.md) |
| **P1** | **New-tenant standup isn't turnkey.** Creating the two application Entra app registrations (API + web SPA) is a manual portal/CLI step. | Optional `scripts/provision-entra-apps.ps1` to create both apps, expose `access_as_user`, set redirect URIs + admin consent, and emit the `AI4IA_ENTRA_*` values. | [`deployment.md` §2.7](./runbooks/deployment.md) |
