...
| **P1-7 — the tested artifact is not the *PR-tested* artifact.** *Partly closed:* all service base images are digest-pinned, PR CI builds all three images, and `deploy.yml` builds once and deploys each exact ACR digest. The proxy image also has a blocking HIGH/CRITICAL scan, SPDX SBOM, and retained build metadata. **Still open:** production images are rebuilt after merge rather than promoted from PR-tested bytes; no image is signed or carries attested provenance; app API/web images lack the proxy's SBOM and blocking image scan. | Publish PR-built images to a staging registry under a PR-scoped identity, then promote the reviewed digests after merge. Add signing/attested provenance and bring API/web evidence up to the proxy image's level. | Letting PR code push to a registry the production apps pull from is a **security-posture change**: a malicious or broken PR could publish there, and the PR identity would need push rights. | The residual trust-boundary decision remains with the owner; the mechanical digest, proxy scan, SBOM, and build-evidence controls are shipped. |
