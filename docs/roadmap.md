...
| **P2** | **`main` branch protection does not yet require the app/infra checks.** The three formerly path-filtered workflows (`app-ci`, `infra-validate`, `docker-build`) now run on **every** PR, so their six contexts — `web`, `api`, `bicep-lint-build`, `web image`, `api image`, `dockerignore context boundary` — are always reported and are safe to require. `scripts/tests/test_gating_workflows.py` pins that property. Until the ruleset is updated, a PR that breaks the API suite or the Bicep build still shows red but is not *blocked*. | Add those six contexts to the ruleset's required list. Verify first, on a docs-only PR, that all six actually report — a required context that is never reported blocks every PR forever. | Owner (repo settings), [`AGENTS.md`](../AGENTS.md) |
