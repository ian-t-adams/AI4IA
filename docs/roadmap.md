...
| **Ops** | **Speech Voice Live remains off.** Azure OpenAI realtime is enabled, but the second `speech_voice_live` provider is off in both the checked-in and live profile. APIM children and account roles retained from an earlier trial are dormant privilege because incremental ARM does not delete omitted resources. | Before enabling, close the managed-identity audience gate and run the signed-in microphone canary. If the provider will remain off, perform a separately approved targeted teardown of only its retained APIM/RBAC objects. | [`deployment.md` §7.3](./runbooks/deployment.md), [`feature-enablement.md`](./runbooks/feature-enablement.md) |
