...
| **Ops** | **PostgreSQL decommission (last step).** Retired from the repo on 2026-08-06 (#293) and the live server **stopped 2026-08-07** after verifying it had taken no connections for 7 days. | **The clock is running:** a stopped Flexible Server **auto-restarts after 7 days** and silently resumes billing, so stopping is a cost pause, not a decommission. Delete it — or re-issue `stop` — before **2026-08-14**. ⚠️ Deletion **destroys the 7-day backup window with it**; unlike Cosmos there is no post-deletion point-in-time restore. | [`memory-migration.md` "Retirement"](./runbooks/memory-migration.md) |
