...
| **Ops** | **PostgreSQL decommission (last step).** Retired from the repo on 2026-08-06 (#293): no IaC provisions it, no setting reads it, and `scripts/tests/test_postgres_retired.py` fails if any of that returns. The **live server still exists** and still bills. | Stop it first (`az postgres flexible-server stop`) — reversible, ~85% cost saving — then delete after the observation window. ⚠️ Deleting a Flexible Server **destroys its 7-day backup window with it**; unlike Cosmos there is no post-deletion point-in-time restore. | [`memory-migration.md` "Retirement"](./runbooks/memory-migration.md) |
