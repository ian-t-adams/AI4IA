...
``model``/``toolbox``/``steps[].{name,instructions,tools}``). azure-ai-projects 2.5.0 has no
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("\n(validation-only -- there is no --create; azure-ai-projects 2.5.0's actual routines")
