...
def render_photo_avatar_policy(catalog: dict[str, Any]) -> str:
    ...
    # The runtime forms this expression relies on, as APIM supplies them:
    # - context.Api.Path carries a leading slash ("/ai4ia-photo-avatars-v1"), although
    #   the API's ARM `path` has none. A slashless comparison refused every call in
    #   production (2026-09-26). The guard trims slashes, then compares exactly, so
    #   "/ai4ia-photo-avatars-v10" and "/ai4ia-photo-avatars-v1/x" still fail.
    # - context.Request.OriginalUrl.Path is the caller's full path, leading slash and
    #   API suffix included ("/ai4ia-photo-avatars-v1/features"): the form that
    #   simplel7proxy_inbound_pre_32.xml also binds.
    # - context.Subscription.Id is the subscription's name (its sid), the value
    #   GatewayLogs records as apimSubscriptionId; gateway.bicep substitutes it.
    validation = (
        "@{\n"
        f'    if ((context.Api.Path ?? "").Trim(\'/\') != "{PHOTO_AVATAR_API_PATH}" ||\n'
    ...
    # A literal apostrophe is legal in this double-quoted attribute and is the form the
    # deployed hand-authored policies already use (TrimStart('?')); html.escape would
    # otherwise emit a numeric character reference no deployed policy relies on.
    validation_attribute = html.escape(validation, quote=True).replace("&#x27;", "'")
    return (
        "<policies>\n"
        "  <inbound>\n"
        ...
        "         to the API-scoped proxy subscription; no caller supplies it. APIM supplies\n"
        "         context.Api.Path with a leading slash, so the guard trims slashes before its\n"
        "         exact comparison. OriginalUrl.Path is the caller's full path, leading slash and\n"
        "         API suffix included; Subscription.Id is the subscription name. -->\n"
        f'    <set-variable name="photoAvatarOperation" value="{validation_attribute}" />\n'
        "    <choose>\n"
