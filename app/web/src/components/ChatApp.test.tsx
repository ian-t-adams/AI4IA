...
describe("ChatApp uploads", () => {
  ...
  // ConversationInspector syncs its local prompt draft from the systemPrompt
  // prop via a useEffect (`useEffect(() => setPromptDraft(systemPrompt),
  // [systemPrompt])`), so any commit of ChatApp's systemPrompt state --
  // including a stale slash-command reconciliation -- can blow away
  // in-progress, never-saved typing in the textarea, not just a previously
  // saved value. onSystemPromptDraftChange bumps
  // systemPromptMutationGenerationRef on every keystroke specifically so an
  // unsaved edit is protected too. This test never clicks Save, isolating
  // that keystroke-level guard from the Save-triggered onSessionUpdated path
  // already covered above.
  it("keeps an unsaved, in-progress system-prompt edit from being overwritten by a stale slash finalize that resolves before Save is ever clicked", async () => {
    const current = new Map([["A", session("A")]]);
    mocks.listSessions.mockImplementation(async () => [...current.values()]);
    mocks.updateSession.mockImplementation(
      async (id: string, value: Record<string, unknown>) => {
    ...
    // Type into the prompt box but never click Save -- the stale
    // reconciliation below must not be able to stomp this unsaved draft.
    await user.click(screen.getByRole("tab", { name: "Instructions" }));
    const prompt = await screen.findByRole("textbox", { name: "System prompt" });
    await user.clear(prompt);
    await user.type(prompt, "Unsaved draft");
    expect(prompt).toHaveValue("Unsaved draft");

    await act(async () => {
      resolveStaleList([
        { ...session("A"), title: "Stale A", systemPrompt: "Stale prompt" },
      ]);
      await Promise.resolve();
    });
