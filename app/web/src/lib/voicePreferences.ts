...
export function usePersistedVoicePreferences(): {
  ...
  useEffect(() => {
    // Client-only hydration after SSR. The single post-mount update is deliberate:
    // reading localStorage during the initial render would create a hydration mismatch.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setState({
      preferences: loadVoicePreferences(),
      hasStoredPreferences: hasStoredVoicePreferences(),
    });
  }, []);

  const updatePreferences = useCallback((next: VoicePreferences) => {
    setState({ preferences: next, hasStoredPreferences: true });
