...
async def test_external_claude_profile_is_bound_by_real_publication_and_consent(publication):
    ...
    model = next(entry for entry in state.catalog.models if entry.deploymentTarget == "external-claude")
    author = await actor(publication, "claude-author", "Author")
    source = await state.agent_service.create(
        author.owner_id, UserAgentCreate(name="claude-helper", systemPrompt="Synthetic.", tools=["calculator"]),
        reserved_names=set(),
    )
    ...
    original = model.reasoningEffort
    model.reasoningEffort = ["low"]
    assert version.profiles["chat"].environmentDigest != environment_hash(state)
    with pytest.raises(PublicationError, match="contract_changed"):
        await publication._check_compilation(author, version)
    model.reasoningEffort = original
    await publication._check_compilation(author, version)
    model.anthropicThinking = None
    with pytest.raises(PublicationError, match="contract_changed"):
        await publication._check_compilation(author, version)
    model.anthropicThinking = "disabled"
    await publication._check_compilation(author, version)
